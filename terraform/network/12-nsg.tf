...
resource "azurerm_network_security_rule" "apim_nsg_rules" {
  ...
  priority                    = lookup(var.apim_nsg_rules[count.index], "priority", "${4096 - length(var.apim_nsg_rules) + count.index }" )
  ...
  description                 = element(var.apim_rules["${lookup(var.apim_nsg_rules[count.index], "name")}"], 5)
  source_address_prefix       = element(var.apim_rules["${lookup(var.apim_nsg_rules[count.index], "name")}"], 6)
  destination_address_prefix  = element(var.apim_rules["${lookup(var.apim_nsg_rules[count.index], "name")}"], 7)
  resource_group_name         = var.resource_group
